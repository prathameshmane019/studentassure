...
const FeedbackForm = ({ params }) => {
  ...
  useEffect(() => {
    const fetchFeedback = async () => {
      ...
      try {
        ...
        setFormData({
          feedback_id: feedbackId,
          responses: response.data.questions.map(() => ({
            ratings: [],
            suggestions: '',
          })),
        });
  ...
  const handleSuggestionsChange = (questionIndex, suggestions) => {
    setFormData((prevFormData) => {
      const updatedResponses = [...prevFormData.responses];
      updatedResponses[questionIndex] = {
        ...updatedResponses[questionIndex],
  ...
  return (
    <div className="flex flex-col items-center min-h-screen bg-gray-100 p-4">
      <div className="w-full max-w-3xl bg-white shadow-md rounded-lg p-8">
        <h2 className="text-3xl font-bold mb-6 text-center">{selectedFeedback.feedbackTitle}</h2>
        {error && <p className="text-red-600 text-center mb-4">{error}</p>}
        <form onSubmit={handleSubmit}>
            ...
            <React.Fragment key={qIndex}>
              <div className="mb-6">
                ...
                <RadioGroup
                  ...
                  value={formData.responses[qIndex].ratings?.toString() || ''}
                  className="flex space-x-4 mt-2"
                >
                  {[1, 2, 3, 4, 5].map((rating) => (
                    <div key={rating} className="flex items-center">
                      <RadioGroupItem
                        ...
                        required
                      />
                      <Label htmlFor={`${qIndex}-${rating}`}>{rating}</Label>
                    </div>
                  ))}
                </RadioGroup>
              </div>
          ...
          <div className="mb-6">
            ...
            <Textarea
              id="suggestions"
              value={formData.responses[selectedFeedback.questions.length]?.suggestions || ''}
              onChange={(e) => handleSuggestionsChange(selectedFeedback.questions.length, e.target.value)}
              className="mt-2"
            />
          </div>
          <Button
